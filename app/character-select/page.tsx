...
export default function CharacterSelect() {
  ...
  const characters = [
    ...
    { id: "locked", label: "Locked Character", image: "/avatars/ascii.png", unlocked: false },
  ];

  return (
    <div className="flex justify-center items-center min-h-screen">
      <div className="nes-container with-title is-rounded">
        <h1 className="title text-lg retro-gradient" style={{marginTop:"0.25rem"}}>Select Your Character</h1>
          ...
          {characters.map((character) => (
            <div
              ...
              className={`character-card nes-container is-rounded flex flex-col items-center justify-between p-2 w-32 h-48 relative overflow-hidden
                         ${!character.unlocked ? "opacity-50 pointer-events-none" : "cursor-pointer"}`} // Added cursor-pointer for unlocked
            >
              {character.unlocked && (
              ...
              <Image
                ...
                width={60}
                height={60}
                className="object-contain mb-2 z-10" // Added z-10 to keep image above hover effect
              />
              <h2 className="text-xs text-center mb-2 z-10">{character.label}</h2> {/* Added z-10 */}

              {character.unlocked ? (
                <button
                  className="nes-btn is-primary mt-auto text-xs px-2 py-1 z-10" // Added z-10
